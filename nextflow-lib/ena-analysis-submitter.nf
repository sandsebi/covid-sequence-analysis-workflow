...
process ena_analysis_submit {
    ...
    if [ "${study_accession}" = 'PRJEB45555' ]; then
        analysis_submission.py -t ${test_submission} -o ${run_accession}_output/${study_accession} -p PRJEB55349 -s ${sample_accession} -r ${run_accession} -f ${output_tgz} -a PATHOGEN_ANALYSIS -au \${webin_id} -ap \${webin_password} -as \${async_flag}
        ...
        analysis_submission.py -t ${test_submission} -o ${run_accession}_output/${study_accession} -p PRJEB55355 -s ${sample_accession} -r ${run_accession} -f ${filtered_vcf_gz} -a COVID19_FILTERED_VCF -au \${webin_id} -ap \${webin_password} -as \${async_flag}
        analysis_submission.py -t ${test_submission} -o ${run_accession}_output/${study_accession} -p PRJEB55356 -s ${sample_accession} -r ${run_accession} -f ${consensus_fasta_gz} -a COVID19_CONSENSUS -au \${webin_id} -ap \${webin_password} -as \${async_flag}
    else
