...
process ena_analysis_submit {
    ...
    file(output_bam)
    file(output_vcf)
    file(output_coverage)
    file(output_annot_vcf)
    ...
    file("${run_accession}_output/${study_accession}/${run_accession}.vcf")
    file("${run_accession}_output/${study_accession}/${run_accession}.coverage")
    file("${run_accession}_output/${study_accession}/${run_accession}.annot.vcf")
    ...
    if [ "${study_accession}" = 'PRJEB45555' ]; then
        analysis_submission.py -t ${test_submission} -o ${run_accession}_output/${study_accession} -p PRJEB55349 -s ${sample_accession} -r ${run_accession} -f ${output_bam} -a PATHOGEN_ANALYSIS -au \${webin_id} -ap \${webin_password}
        analysis_submission.py -t ${test_submission} -o ${run_accession}_output/${study_accession} -p PRJEB55349 -s ${sample_accession} -r ${run_accession} -f ${output_coverage} -a PATHOGEN_ANALYSIS -au \${webin_id} -ap \${webin_password}
        analysis_submission.py -t ${test_submission} -o ${run_accession}_output/${study_accession} -p PRJEB55349 -s ${sample_accession} -r ${run_accession} -f ${output_vcf} -a PATHOGEN_ANALYSIS -au \${webin_id} -ap \${webin_password}
        analysis_submission.py -t ${test_submission} -o ${run_accession}_output/${study_accession} -p PRJEB55349 -s ${sample_accession} -r ${run_accession} -f ${output_annot_vcf} -a PATHOGEN_ANALYSIS -au \${webin_id} -ap \${webin_password}
        analysis_submission.py -t ${test_submission} -o ${run_accession}_output/${study_accession} -p PRJEB54593 -s ${sample_accession} -r ${run_accession} -f ${filtered_vcf_gz} -a COVID19_FILTERED_VCF -au \${webin_id} -ap \${webin_password}
    ...
    mv ${output_bam} ${output_coverage} ${output_vcf} ${output_annot_vcf} ${filtered_vcf_gz} ${consensus_fasta_gz} ${run_accession}_output/${study_accession}
